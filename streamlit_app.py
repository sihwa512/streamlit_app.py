...
# --- 1. 頁面設定 ---
st.set_page_config(page_title="綜合退休戰情室 V72.7", layout="wide")
...
def save_data_to_gs(stocks, principal):
    ...
    try:
        ...
        # 🌟 寫入核心修正：確保語法相容新舊版 gspread
        # 1. 儲存 Stocks
        ws_s = doc.worksheet("Stocks")
        s_list = [["id", "sh", "co"]] + [[sid, float(v['sh']), float(v['co'])] for sid, v in stocks.items()]
        ws_s.batch_clear(["A1:C100"]) # 先清空區域
        ws_s.update(s_list, "A1")     # 再寫入
        
        # 2. 儲存本金 Settings
        try:
            ws_v = doc.worksheet("Settings")
            ws_v.batch_clear(["A1:B5"])
...
st.markdown("""
    ...
    [data-testid="stMetricValue"] > div { color: #00d4ff !important; font-family: 'Consolas', monospace !important; font-size: 2.5rem !important; font-weight: 800 !important; }
...
with col3:
    ...
    st.metric("累積總損益", f"${pnl:,.0f}", f"{pct:.2f}%")
...
def stat_box(label, color, val, pct):
    return f"<div class='stat-card' style='border-top: 6px solid {color};'><small style='color:#8b949e;'>{label}</small><br><b style='color:{color}; font-size:28px;'>{pct:.1f}%</b><br><b style='color:{color}; font-size:24px;'>${val:,.0f}</b></div>"
...
with cp:
    if total_mkt > 0:
        ...
        fig.update_layout(template="plotly_dark", margin=dict(t=30,b=30,l=30,r=30), paper_bgcolor='rgba(0,0,0,0)')
