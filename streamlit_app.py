...
# --- 1. 核心連線設定 ---
st.set_page_config(page_title="退休戰情室 V78.4", layout="wide")
...
<style>
    ...
    .responsive-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 15px; margin-bottom: 25px; }
    ...
    .info-box { background: #161b22; border-radius: 12px; padding: 22px; text-align: center; border: 1px solid #30363d; }
    ...
    .beta-tag { background: #30363d; color: #ff9f1c; padding: 4px 10px; border-radius: 6px; font-size: 0.9rem; font-family: 'Consolas'; }
...
<div class="responsive-grid">
    ...
    <div class="metric-card"><div class="label-bright">📊 累計損益</div><div class="val-main {'up' if (total_mkt-total_capital)>=0 else 'down'}">${int(total_mkt-total_capital):,}</div><div style="color:#ffffff; font-size:0.9rem;">本金: ${int(total_capital):,}</div></div>
</div>
""", unsafe_allow_html=True)
...
# --- 6. 配置現況與目標 ---
st.subheader("⚖️ 資產配置現況與 Beta 導航")
st.markdown(f"""
<div class="responsive-grid">
    <div class="info-box b-blue"><div class="label-bright">現況 股票</div><div style="font-size:2.2rem; font-weight:900;">{s_p}%</div><div>${int(s_v):,}</div></div>
    <div class="info-box b-purple"><div class="label-bright">現況 槓桿</div><div style="font-size:2.2rem; font-weight:900;">{l_p}%</div><div>${int(l_v):,}</div></div>
    <div class="info-box b-green"><div class="label-bright">現況 類現金</div><div style="font-size:2.2rem; font-weight:900;">{c_p}%</div><div>${int(c_v):,}</div></div>
</div>
...
st.markdown(f"""
<div class="responsive-grid">
    <div class="info-box b-blue" style="background:#1c2128"><div class="label-bright">🎯 目標 股票</div><div style="font-size:1.9rem; font-weight:800; opacity:0.8;">{ts_pct}%</div><div>${int(ts_amt):,}</div></div>
    <div class="info-box b-purple" style="background:#1c2128"><div class="label-bright">🎯 目標 槓桿</div><div style="font-size:1.9rem; font-weight:800; opacity:0.8;">{tl_pct}%</div><div>${int(tl_amt):,}</div></div>
    <div class="info-box b-green" style="background:#1c2128"><div class="label-bright">🎯 目標 類現金</div><div style="font-size:1.9rem; font-weight:800; opacity:0.8;">{tc_pct}%</div><div>${int(tc_amt):,}</div></div>
</div>
<div style="text-align:right; margin-top:-10px; margin-bottom:10px;">
    <span class="beta-tag" style="border:1px solid #ff9f1c">預期目標 Beta: {t_beta:.2f}</span>
</div>
""", unsafe_allow_html=True)
...
if active_data:
    html = "<div style='overflow-x:auto;'><table><thead><tr><th>標的</th><th>持股數</th><th>報價</th><th>Beta</th><th>市值</th><th>報酬</th><th>佔比</th><th>操作建議</th></tr></thead><tbody>"
