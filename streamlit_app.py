...
if not df_snap.empty and len(df_snap) > 1:
    ...
    df_cal['total_mkt'] = pd.to_numeric(df_cal['total_mkt'], errors='coerce').fillna(0)
    
    df_cal['daily_diff'] = df_cal['total_mkt'].diff()
    df_cal['daily_pct'] = (df_cal['daily_diff'] / df_cal['total_mkt'].shift(1)) * 100
    
    df_cal['year'] = df_cal['date'].dt.year
    df_cal['month'] = df_cal['date'].dt.month
    df_cal['day'] = df_cal['date'].dt.day
    df_cal['year_month'] = df_cal['date'].dt.strftime('%Y-%m')

    tab_cal, tab_month, tab_year = st.tabs(["📆 本月日曆", "📊 月報酬", "📈 年報酬"])
    
    with tab_cal:
        curr_year, curr_month = now_tw.year, now_tw.month
        cal_data = df_cal[(df_cal['year'] == curr_year) & (df_cal['month'] == curr_month)]
        
        day_dict = {}
        for _, row in cal_data.iterrows():
            if pd.notna(row['daily_diff']):
                day_dict[int(row['day'])] = {'diff': row['daily_diff'], 'pct': row['daily_pct']}
...
<div style="color:#8b949e; font-size:0.9rem; text-align:left;">{day}</div>
    ...
    with tab_month:
        ...
        month_end['month_diff'] = month_end['total_mkt'].diff()
        month_end['month_pct'] = (month_end['month_diff'] / month_end['total_mkt'].shift(1)) * 100
        
        m_html = "<table><thead><tr><th>月份</th><th>月底總市值</th><th>月損益</th><th>月報酬率</th></tr></thead><tbody>"
        for _, r in month_end.dropna(subset=['month_diff']).sort_values('year_month', ascending=False).iterrows():
            m_color = "up" if r['month_diff'] >= 0 else "down"
    ...
    with tab_year:
        ...
        year_end['year_diff'] = year_end['total_mkt'].diff()
        year_end['year_pct'] = (year_end['year_diff'] / year_end['total_mkt'].shift(1)) * 100
        
        y_html = "<table><thead><tr><th>年度</th><th>年底總市值</th><th>年損益</th><th>年報酬率</th></tr></thead><tbody>"
        for _, r in year_end.dropna(subset=['year_diff']).sort_values('year', ascending=False).iterrows():
            y_color = "up" if r['year_diff'] >= 0 else "down"
    ...
    if st.button("💾 永久保存設定", use_container_width=True):
        ...
        if client:
            try:
                ...
                safe_token = st.session_state.get("api_token", line_channel_token)
                safe_uid = st.session_state.get("api_uid", line_user_id)
                ...
                ws_settings.append_rows([
                    ...
                    ["line_channel_token", safe_token], ["line_user_id", safe_uid] 
                ])
...
with st.sidebar:
    ...
    channel_token_input = st.text_input("Channel Access Token", value=line_channel_token, type="password", key="api_token")
    user_id_input = st.text_input("您的 User ID", value=line_user_id, type="password", key="api_uid")
    
    if st.button("💾 儲存 LINE 金鑰", use_container_width=True):
        client = get_client()
        if client:
            try:
                ws_settings = client.open_by_key(GS_ID).worksheet("Settings")
                ...
                ws_settings.append_rows([
                    ...
                    ["goal_amt", goal_amt], ["deviation_band", deviation_band],
                    ["borrowed_amt", borrowed_input], ["expected_yield", yield_input],
                    ["line_channel_token", channel_token_input], ["line_user_id", user_id_input] 
                ])
                st.cache_data.clear(); st.rerun()
            except Exception as e: st.error(f"雲端儲存失敗: {e}")
    ...
    if st.button("📢 發送今日戰報", use_container_width=True):
        if not channel_token_input or not user_id_input:
            st.error("請填寫 Token 與 User ID！並點擊上方的『儲存 LINE 金鑰』。")
        else:
