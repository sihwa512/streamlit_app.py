...
import re
import pytz # 🌟 必須加入時區處理

# --- 1. 核心連線設定 ---
st.set_page_config(page_title="退休戰情室 V81.0", layout="wide")
GS_ID = "1jgZhEi-nmaXGUa5fJaYwk79xE9-QG4LwhwV89xriGPs"
TW_TIMEZONE = pytz.timezone('Asia/Taipei') # 🌟 設定台灣時區
...
def fetch_cloud_data():
    ...
    try:
        ...
        try:
            ws_snap = doc.worksheet("DailySnapshots")
            df_snap = pd.DataFrame(ws_snap.get_all_records())
        except:
...
<style>
    ...
    .metric-card { background: #1e242b; border: 1px solid #3d4754; border-radius: 12px; padding: 16px; text-align: center; }
    .label-bright { color: #8b949e !important; font-size: 1.0rem; font-weight: 500; }
    ...
    .info-box { background: #1c2128; border-radius: 12px; padding: 18px; text-align: center; border: 1px solid #30363d; }
    .box-pct { font-size: 1.9rem; font-weight: 800; line-height: 1.2; color: #f0f6fc; }
    .b-blue { border-top: 5px solid #4493f8; } .b-purple { border-top: 5px solid #ab7df8; } .b-green { border-top: 5px solid #3fb950; }
    .beta-tag { background: #2d333b; color: #d29922; padding: 4px 10px; border-radius: 6px; font-size: 0.9rem; font-family: 'Consolas'; border: 1px solid #444c56; }
    table { width: 100%; border-collapse: collapse; font-size: 1.1rem !important; }
    th { background: #21262d !important; color: #8b949e !important; padding: 12px !important; }
    td { padding: 12px !important; border-bottom: 1px solid #30363d !important; }
    .up { color: #f85149; font-weight: 600; } .down { color: #3fb950; font-weight: 600; }
</style>
...
<div class="metric-grid">
    ...
    <div class="metric-card"><div class="label-bright">💰 資產總市值</div><div class="val-main" style="color:#56d364">${int(total_mkt):,}</div><div class="val-sub">昨日紀錄: ${int(yesterday_mkt):,}</div></div>
    <div class="metric-card"><div class="label-bright">📈 今日損益變動 (自動)</div><div class="val-main {'up' if snap_delta>=0 else 'down'}">${int(snap_delta):,}</div><div class="val-sub">基於跨日快照對比</div></div>
    <div class="metric-card"><div class="label-bright">📊 真實累積盈虧</div><div class="val-main {'up' if (total_mkt-total_capital)>=0 else 'down'}">${int(total_mkt-total_capital):,}</div><div class="val-sub">本金: ${int(total_capital):,}</div></div>
</div>
""", unsafe_allow_html=True)

with st.sidebar:
    st.header("🎯 闖關目標設定")
    goal_amt = st.number_input("設定財務自由目標", value=30000000, step=1000000)
...
fig_prog.add_trace(go.Bar(x=[total_mkt], y=["進度"], orientation='h', marker=dict(color='#56d364'), text=[f"目前: ${int(total_mkt):,}"], textposition='inside', insidetextanchor='middle', textfont=dict(color='#0d1117', family='Consolas')))
...
fig_prog.update_layout(barmode='overlay', xaxis=dict(range=[0, max_x], visible=False), yaxis=dict(visible=False), height=120, margin=dict(l=10, r=10, t=55, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False)
