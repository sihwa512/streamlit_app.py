...
# --- 1. 頁面設定 ---
st.set_page_config(page_title="綜合退休戰情室 V72.6", layout="wide")
...
st.markdown("""
    ...
    [data-testid="stMetricLabel"] > div { color: #ffffff !important; font-weight: bold !important; opacity: 1 !important; }
    
    /* 方塊樣式 */
    .stat-card { 
        text-align: center; padding: 20px; background: #161b22; 
        border-radius: 12px; border: 1px solid #30363d; margin-bottom: 15px; 
    }
    
    h1, h2, h3 { color: #58a6ff !important; font-weight: 800 !important; }
...
def stat_box(label, color, val, pct):
    return f"""<div class='stat-card' style='border-top: 6px solid {color};'>
...
with t3:
    ...
    st.markdown(f"<div style='text-align:center;'><small style='color:#aaa;'>目標 類現金</small><br><b style='color:#3fb950; font-size:24px;'>{t_safe_pct}%</b></div>", unsafe_allow_html=True)

st.markdown(f"""<div style='display:flex; justify-content:space-around; background:rgba(255,159,28,0.1); padding:12px; border-radius:10px; border:1px solid #ff9f1c;'>
...
with col_pie:
    ...
    if total_mkt > 0:
        ...
        fig.update_layout(template="plotly_dark", margin=dict(t=30,b=30,l=30,r=30), paper_bgcolor='rgba(0,0,0,0)', showlegend=True)
